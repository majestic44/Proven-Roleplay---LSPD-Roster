(() => {
  ...
  function getActiveData() {
    return activeTab === "ten" ? TEN_CODES : RESPONSE_CODES;
  }

  function matches([code, desc], q) {
    if (!q) return true;
    const hay = `${String(code)} ${String(desc)}`.toLowerCase();
  ...
  function render() {
    ...
    const rows = data.filter((row) => matches(row, q));
    codesBody.innerHTML = "";
    ...
    for (const [code, desc] of rows) {
      ...
      tdCode.innerHTML = `<span class="codePill">${escapeHtml(code)}</span>`;

      const tdDesc = document.createElement("td");
      tdDesc.innerHTML = `<div class="desc">${escapeHtml(desc)}</div>`;
    ...
    countText.textContent = `${rows.length} ${rows.length === 1 ? "entry" : "entries"}`;
  }
